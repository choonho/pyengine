...
class Command(object):
    ...
    def makeSearch(self, *args):
        ...
        """
        if self.params.has_key('search') == False:
            self.params['search'] = []
        ...
        for s in search:
            if s['key'] in global_conf.DATETIME_FIELDS:
                try:
    ...
    def makeSort(self, field):
        """
        Set Default Sorting
        """
        if self.params.has_key('sort') == False:
            self.params['sort'] = {'field': field}

        return self.params['sort']

    def makeMinimal(self):
        if self.params.has_key('minimal') == False:
            self.params['minimal'] = False

        return self.params['minimal']
